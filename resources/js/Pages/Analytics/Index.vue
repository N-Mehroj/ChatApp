<template>
	<h2 class="mt-10 text-lg font-medium intro-y">Список трекингов и аналитик</h2>
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div
			class="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap"
		>
			<Button variant="primary" class="mr-2 shadow-md">
				<Link href="/analytics/create">
					Добавить трекинг
				</Link>
			</Button>
			<Menu>
			...
			<div v-if="analytics.total" class="hidden mx-auto md:block text-slate-500">
				Показано с {{ analytics.from }} по {{ analytics.to }} из {{ analytics.total }} записей
			</div>
			<div v-else class="hidden mx-auto md:block text-slate-500">
				Трекинги и аналитики не найдены
			</div>
			<Search />
		</div>
		<template v-if="analyticsList.length">
			<!-- BEGIN: Data List -->
			<div class="col-span-12 overflow-x-auto intro-y">
				<Table class="border-spacing-y-[10px] border-separate -mt-2">
					...
					<Table.Tbody>
						<Table.Tr
							v-for="analytic in analyticsList"
							:key="analytic.id"
							class="intro-x"
						>
							<Table.Td
							...
							>
								<div class="flex items-center justify-center">
									<Link class="flex items-center mr-3" :href="`/analytics/${analytic.id}/edit`">
									...
									<a
										...
										href="#"
										@click.prevent="deleteUser(analytic.id)"
									>
...
export default {
	...
	computed: {
		analyticsList() {
			return this.analytics.data
		}
	},
	methods: {
		deleteUser(id) {
			this.deleteRecordUrl = `/analytics/${id}`
