<template>
	<h2 class="mt-10 text-lg font-medium intro-y">Все баннерные зоны</h2>
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div
			class="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap"
		>
			<Button variant="primary" class="mr-2 shadow-md">
				<Link href="/advertising/banner-positions/create">
					Добавить баннерную зону
				</Link>
			</Button>
			<Menu>
			...
			<div v-if="bannerPositions.total" class="hidden mx-auto md:block text-slate-500">
				Показано с {{ bannerPositions.from }} по {{ bannerPositions.to }} из {{ bannerPositions.total }} записей
			</div>
		...
		<template v-if="bannerPositionsList.length">
			<!-- BEGIN: Data List -->
				...
				<Table class="border-spacing-y-[10px] border-separate -mt-2">
					<Table.Thead>
						<Table.Tr>
							...
							<Table.Th class="border-b-0 whitespace-nowrap">Позиция</Table.Th>
							<Table.Th class="text-center border-b-0 whitespace-nowrap">Действия</Table.Th>
						</Table.Tr>
					</Table.Thead>
					<Table.Tbody>
						<Table.Tr
							v-for="bannerPosition in bannerPositionsList"
							:key="bannerPosition.id"
							class="intro-x"
						>
							<Table.Td
							...
							>
								<div class="flex items-center justify-center">
									<Link class="flex items-center mr-3" :href="`/advertising/banner-positions/${bannerPosition.id}/edit`">
									...
									<a
										...
										href="#"
										@click.prevent="deleteRecord(bannerPosition.id)"
									>
			...
			>
				<ThePagination :data="bannerPositions" />
			</div>
			<!-- END: Pagination -->
		</template>
	</div>
...
export default {
	...
	computed: {
		bannerPositionsList() {
			return this.bannerPositions.data
		}
	},
	methods: {
		deleteRecord(id) {
			this.deleteRecordUrl = `/advertising/banner-positions/${id}`
