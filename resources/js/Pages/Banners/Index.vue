<template>
	<h2 class="mt-10 text-lg font-medium intro-y">Все баннеры</h2>
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div
			class="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap"
		>
			<Button variant="primary" class="mr-2 shadow-md">
				<Link href="/advertising/banners/create">
					Добавить баннер
				</Link>
			</Button>
			<Menu>
			...
			<div v-if="banners.total" class="hidden mx-auto md:block text-slate-500">
				Показано с {{ banners.from }} по {{ banners.to }} из {{ banners.total }} записей
			</div>
		...
		<template v-if="bannersList.length">
			<!-- BEGIN: Data List -->
						...
						>
							...
							>
								<div class="flex items-center justify-center">
									<Link class="flex items-center mr-3" :href="`/advertising/banners/${banner.id}/edit`">
									...
									<a
										...
										href="#"
										@click.prevent="deleteRecord(banner.id)"
									>
...
export default {
	...
	methods: {
		deleteRecord(id) {
			this.deleteRecordUrl = `/advertising/banners/${id}`
