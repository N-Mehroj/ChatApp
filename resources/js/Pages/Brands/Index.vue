<template>
	<h2 class="mt-10 text-lg font-medium intro-y">Бренды</h2>
	<div class="grid grid-cols-12 gap-6 mt-5">
		...
		>
			...
			<div v-if="brands.total" class="hidden mx-auto md:block text-slate-500">
		...
		<template v-if="brandsList.length">
			<!-- BEGIN: Data List -->
				...
				<Table class="border-spacing-y-[10px] border-separate -mt-2">
					...
					<Table.Tbody>
						...
						>
							...
							>
								<Link :href="`/brands/${brand.id}/edit`" class="font-medium whitespace-nowrap">
									{{ brand.id }}
								</Link>
							</Table.Td>
							<Table.Td
							...
							>
								<div class="flex items-center justify-center">
									<Link class="flex items-center mr-3 text-primary" :href="`/brands/${brand.id}/edit`">
										<Lucide icon="CheckSquare" class="w-4 h-4 mr-1 text-primary" />
									</Link>
									<a
										class="flex items-center text-danger"
										href="#"
										@click.prevent="deleteRecord(brand.id)"
									>
...
export default {
	...
	methods: {
		deleteRecord(id) {
			this.deleteRecordUrl = `/brands/${id}`
