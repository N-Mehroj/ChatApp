<template>
	<h2 class="mt-10 text-lg font-medium intro-y">Все модели</h2>
	<div class="grid grid-cols-12 gap-6 mt-5">
		...
		>
			...
			<div v-if="groups.total" class="hidden mx-auto md:block text-slate-500">
				Показано с {{ groups.from }} по {{ groups.to }} из {{ groups.total }} записей
			</div>
				...
				<Table class="border-spacing-y-[10px] border-separate -mt-2">
					...
					<Table.Tbody>
						...
						>
							...
							>
								<Link :href="`/groups/${group.id}/edit`" class="font-medium whitespace-nowrap">
									{{ group.id }}
								</Link>
							</Table.Td>
							<Table.Td
							...
							>
								<div class="flex items-center justify-center">
									<Link class="flex items-center mr-3 text-primary" :href="`/groups/${group.id}/edit`">
										<Lucide icon="CheckSquare" class="w-4 h-4 mr-1 text-primary" />
									</Link>
									<a
										class="flex items-center text-danger"
										href="#"
										@click.prevent="deleteRecord(group.id)"
									>
...
export default {
	...
	methods: {
		deleteRecord(id) {
			this.deleteRecordUrl = `/groups/${id}`
