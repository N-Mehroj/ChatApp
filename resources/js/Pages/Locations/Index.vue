<template>
	<h2 class="mt-10 text-lg font-medium intro-y">Все локации</h2>
	<div class="grid grid-cols-12 gap-6 mt-5">
		<div
			class="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap"
		>
			<Button variant="primary" class="mr-2 shadow-md">
				<Link href="/handbook/locations/create">
					Добавить локацию
				</Link>
			</Button>
			<Menu>
			...
			<div v-if="locations.total" class="hidden mx-auto md:block text-slate-500">
				Показано с {{ locations.from }} по {{ locations.to }} из {{ locations.total }} записей
			</div>
		...
		<template v-if="locationsList.length">
			<!-- BEGIN: Data List -->
					...
					<Table.Tbody>
						...
						>
							...
							<Table.Td
								class="first:rounded-l-md last:rounded-r-md bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]"
							>
								{{ location.location_type.label_ru }}
							</Table.Td>
							<Table.Td
							...
							>
								<div class="flex items-center justify-center">
									<Link class="flex items-center mr-3 text-primary" :href="`/handbook/locations/${location.id}/edit`">
										<Lucide icon="CheckSquare" class="w-4 h-4 mr-1 text-primary" />
									</Link>
									<a
										class="flex items-center text-danger"
										href="#"
										@click.prevent="deleteRecord(location.id)"
									>
...
export default {
	...
	methods: {
		deleteRecord(id) {
			this.deleteRecordUrl = `/handbook/locations/${id}`
