<template>
	<h2 class="mt-10 text-lg font-medium intro-y">Все офферы</h2>
	<div class="grid grid-cols-12 gap-6 mt-5">
		...
		>
			...
			<div v-if="offers.total" class="hidden mx-auto md:block text-slate-500">
				Показано с {{ offers.from }} по {{ offers.to }} из {{ offers.total }} записей
			</div>
				...
				<Table class="border-spacing-y-[10px] border-separate -mt-2">
					<Table.Thead>
						<Table.Tr>
							...
							<Table.Th class="border-b-0 whitespace-nowrap">SPU</Table.Th>
					...
					<Table.Tbody>
						...
						>
							...
							>
								<Link :href="`/offers/${offer.id}/edit`" class="font-medium whitespace-nowrap">
									{{ offer.id }}
								</Link>
							</Table.Td>
							<Table.Td
							...
							<Table.Td
								class="first:rounded-l-md last:rounded-r-md bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]"
							>
								<Link v-if="offer.product_id" :href="`/products/${offer.product_id}/edit`" class="font-medium whitespace-nowrap">
									{{ offer.product.name_ru }}
								</Link>
							</Table.Td>
							<Table.Td
							...
							>
								<div class="flex items-center justify-center">
									<Link class="flex items-center mr-3 text-primary" :href="`/offers/${offer.id}/edit`">
										<Lucide icon="CheckSquare" class="w-4 h-4 mr-1 text-primary" />
									</Link>
									<a
										class="flex items-center text-danger"
										href="#"
										@click.prevent="deleteRecord(offer.id)"
									>
...
export default {
	...
	methods: {
		deleteRecord(id) {
			this.deleteRecordUrl = `/offers/${id}`
