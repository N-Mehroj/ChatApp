<template>
	<h2 class="mt-10 text-lg font-medium intro-y">Все роли</h2>
	<div class="grid grid-cols-12 gap-6 mt-5">
		...
		>
			...
			<div v-if="roles.total" class="hidden mx-auto md:block text-slate-500">
				Показано с {{ roles.from }} по {{ roles.to }} из {{ roles.total }} записей
			</div>
					...
					>
						...
						>
							<div class="flex items-center justify-center">
								<Link class="flex items-center mr-3" :href="`/${crudUrl}/${role.id}/edit`">
								...
								<a
									...
									href="#"
									@click.prevent="deleteRecord(role.id)"
								>
