<template>
    <h2 class="mt-10 text-lg font-medium intro-y">Все SEO текста</h2>
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div
            class="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap"
        >
            <Button variant="primary" class="mr-2 shadow-md">
                <Link href="/seo-texts/create">
                    Добавить SEO текст
                </Link>
            </Button>
            <div v-if="seoTexts.total" class="hidden mx-auto md:block text-slate-500">
                Показано с {{ seoTexts.from }} по {{ seoTexts.to }} из {{ seoTexts.total }} записей
            </div>
            <div v-else class="hidden mx-auto md:block text-slate-500">
                SEO текста не найдены
            </div>
            <Search />
        </div>
        <template v-if="seoTextsList.length">
            <!-- BEGIN: Data List -->
                        ...
                        >
                            ...
                            >
                                <div class="flex items-center justify-center">
                                    <Link class="flex items-center mr-3" :href="`/seo-texts/${seoText.id}/edit`">
                                    ...
                                    <a
                                        ...
                                        href="#"
                                        @click.prevent="deleteRecord(seoText.id)"
                                    >
...
export default {
    ...
    computed: {
        seoTextsList() {
            return this.seoTexts.data
        }
    },
    methods: {
        deleteRecord(id) {
            this.deleteRecordUrl = `/seo-texts/${id}`
