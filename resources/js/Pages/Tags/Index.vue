<template>
	<h2 class="mt-10 text-lg font-medium intro-y">Теги</h2>
	<div class="grid grid-cols-12 gap-6 mt-5">
		...
		>
			...
			<div v-if="tags.total" class="hidden mx-auto md:block text-slate-500">
				Показано с {{ tags.from }} по {{ tags.to }} из {{ tags.total }} записей
			</div>
				...
				<Table class="border-spacing-y-[10px] border-separate -mt-2">
					...
					<Table.Tbody>
						...
						>
							...
							>
								<Link :href="`/tags/${tag.id}/edit`" class="font-medium whitespace-nowrap">
									{{ tag.id }}
								</Link>
							</Table.Td>
							<Table.Td
							...
							>
								<div class="flex items-center justify-center">
									<Link class="flex items-center mr-3 text-primary" :href="`/tags/${tag.id}/edit`">
										<Lucide icon="CheckSquare" class="w-4 h-4 mr-1 text-primary" />
									</Link>
									<a
										class="flex items-center text-danger"
										href="#"
										@click.prevent="deleteRecord(tag.id)"
									>
