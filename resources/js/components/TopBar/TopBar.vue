    ...
    <div class="flex items-center h-full">
      ...
      >
        <img
          ...
          :src="logoUrl"
        />
        <span class="ml-3 text-lg text-white"> Platform </span>
      </RouterLink>
      <!-- END: Logo -->
      <!-- BEGIN: Breadcrumb -->
      <Breadcrumb
      ...
      >
        <Breadcrumb.Link to="/">Приложение</Breadcrumb.Link>
      </Breadcrumb>
      <!-- END: Breadcrumb -->
      <!-- BEGIN: Search -->
        ...
        <Menu>
            ...
            >
                <Menu.Header class="font-normal">
                    <div class="font-medium">{{ $page.props.auth.name }}</div>
                    <div class="text-xs text-white/70 mt-0.5 dark:text-slate-500">
                        {{ $page.props.auth.roles && $page.props.auth.roles[0].label_ru }}
                    </div>
                </Menu.Header>
                <Menu.Devider class="bg-white/[0.08]" />
                <Link :href="`/users/${$page.props.auth.id}/edit`"
                      class="hover:bg-white/5 cursor-pointer flex items-center p-2 transition duration-300 ease-in-out rounded-md dark:bg-darkmode-600 dark:hover:bg-darkmode-400">
                    <Lucide icon="User" class="w-4 h-4 mr-2" /> Профиль
                </Link>
                <Menu.Devider class="bg-white/[0.08]" />
                <Link href="/logout"
                      method="post"
                      class="hover:bg-white/5 cursor-pointer flex items-center p-2 transition duration-300 ease-in-out rounded-md dark:bg-darkmode-600 dark:hover:bg-darkmode-400">
                    <Lucide icon="ToggleRight" class="w-4 h-4 mr-2" /> Выйти
                </Link>
            </Menu.Items>
        </Menu>
